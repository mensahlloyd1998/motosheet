...
<title>Edit {{ $car->make }} {{ $car->model }} {{ $car->trim }} — Motosheet</title>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  #toast{position:fixed;top:20px;right:20px;z-index:1000;display:flex;flex-direction:column;gap:10px;}
  #toast .toast-item{min-width:240px;max-width:340px;padding:13px 16px;border-radius:6px;font-size:13.5px;font-weight:500;box-shadow:0 8px 24px rgba(13,13,13,0.12);animation:slideIn .2s ease;}
  ...
  .photo-thumb .set-cover-btn{
    position:absolute;bottom:6px;left:6px;background:rgba(255,255,255,0.92);color:var(--black);
    border:none;border-radius:100px;font-size:10px;font-weight:700;padding:3px 8px;letter-spacing:0.03em;text-transform:uppercase;
  }
  .photo-thumb .set-cover-btn:hover{background:#fff;color:var(--orange-deep);}
  .photo-subhead{font-size:12.5px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:var(--text-muted);margin:22px 0 10px;}
  .photo-subhead:first-child{margin-top:0;}
  .empty-photos-note{font-size:13px;color:var(--text-muted);padding:14px 0;}
...
<main class="wrap" style="flex:1;">
    ...
    <div class="eyebrow-row">
        <a href="{{ route('cars.index') }}">My Listings</a><span class="sep">/</span>
        <a href="{{ route('cars.show', ['car' => $car->id]) }}">{{ $car->make }} {{ $car->model }} {{ $car->trim }}</a><span class="sep">/</span>
        <span class="current">Edit</span>
    </div>

    <div class="page-head">
        <h1>Edit Vehicle</h1>
        <p class="sub">Update the details below. Changes are saved to your existing listing — its link and QR code stay the same.</p>
    </div>
    ...
    <form method="post" action="{{ route('cars.update', ['car' => $car->id]) }}" enctype="multipart/form-data" id="carForm">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <div class="section-card">
                    ...
                    <div class="field-grid">
                        <div class="field">
                            ...
                            <input type="text" id="make" name="make" value="{{ old('make', $car->make) }}" placeholder="e.g. Porsche" required>
                            @error('make') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                        <div class="field">
                            <label for="model">Model</label>
                            <input type="text" id="model" name="model" value="{{ old('model', $car->model) }}" placeholder="e.g. 911" required>
                    ...
                    <div class="field-grid">
                        <div class="field">
                            ...
                            <input type="text" id="trim" name="trim" value="{{ old('trim', $car->trim) }}" placeholder="e.g. Carrera S">
                            @error('trim') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                        <div class="field">
                            <label for="year">Year Model</label>
                            <input type="number" id="year" name="year" value="{{ old('year', $car->year) }}" placeholder="e.g. 2022" min="1900" max="{{ $currentYear }}" required>
                    ...
                    <div class="field-grid cols-3">
                        <div class="field">
                            ...
                            @php $selCondition = old('condition', $car->condition); @endphp
                            <select id="condition" name="condition" required>
                                <option value="" disabled {{ $selCondition ? '' : 'selected' }}>Select</option>
                                <option value="new" {{ $selCondition === 'new' ? 'selected' : '' }}>Brand New</option>
                                <option value="used" {{ $selCondition === 'used' ? 'selected' : '' }}>Used</option>
                                <option value="foreign_used" {{ $selCondition === 'foreign_used' ? 'selected' : '' }}>Foreign Used</option>
                            </select>
                            @error('condition') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                        <div class="field">
                            <label for="transmission">Transmission</label>
                            @php $selTransmission = old('transmission', $car->transmission); @endphp
                            <select id="transmission" name="transmission" required>
                                <option value="" disabled {{ $selTransmission ? '' : 'selected' }}>Select</option>
                                <option value="manual" {{ $selTransmission === 'manual' ? 'selected' : '' }}>Manual</option>
                                <option value="automatic" {{ $selTransmission === 'automatic' ? 'selected' : '' }}>Automatic</option>
                            </select>
                            @error('transmission') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                        <div class="field">
                            <label for="fuel_type">Fuel Type</label>
                            @php $selFuel = old('fuel_type', $car->fuel_type); @endphp
                            <select id="fuel_type" name="fuel_type" required>
                                <option value="" disabled {{ $selFuel ? '' : 'selected' }}>Select</option>
                                <option value="petrol" {{ $selFuel === 'petrol' ? 'selected' : '' }}>Petrol</option>
                                <option value="diesel" {{ $selFuel === 'diesel' ? 'selected' : '' }}>Diesel</option>
                                <option value="hybrid" {{ $selFuel === 'hybrid' ? 'selected' : '' }}>Hybrid</option>
                                <option value="electric" {{ $selFuel === 'electric' ? 'selected' : '' }}>Electric</option>
                            </select>
                    ...
                    <div class="field-grid">
                        <div class="field">
                            ...
                            <input type="number" id="mileage" name="mileage" value="{{ old('mileage', $car->mileage) }}" placeholder="e.g. 8200" min="0" required>
                        ...
                        <div class="field">
                            ...
                            <input type="text" id="color" name="color" value="{{ old('color', $car->exterior_color) }}" placeholder="e.g. GT Silver Metallic">
                    ...
                    <div class="price-row">
                        <div class="field">
                            ...
                            <div class="price-input-wrap">
                                <span>{{ $currency }}</span>
                                <input type="number" id="price" name="price" value="{{ old('price', $car->price) }}" placeholder="0.00" min="0" step="0.01" required>
                            </div>
                        ...
                        <div class="field">
                            ...
                            <div class="toggle-row">
                                ...
                                <label class="switch">
                                    ...
                                    <input type="checkbox" id="is_negotiable" name="is_negotiable" value="1" {{ old('is_negotiable', $car->price_type === 'negotiable' ? 1 : 0) ? 'checked' : '' }}>
                ...
                <div class="section-card">
                    ...
                    <div class="field">
                        ...
                        <textarea id="description" name="description" placeholder="This vehicle has been carefully maintained...">{{ old('description', $car->description) }}</textarea>
                ...
                <div class="section-card">
                    ...
                    <div id="featuresList">
                        @php
                            $existingFeatures = is_array($car->features) && count($car->features) ? $car->features : [''];
                            $oldFeatures = old('features', $existingFeatures);
                        @endphp
                ...
                <div class="section-card">
                    ...
                    <p class="section-desc">At least 5 photos are required in total, across your current photos and any you add below.</p>

                    <div class="photo-subhead">Current Photos ({{ $car->images->count() }})</div>
                    <div class="photo-grid" id="existingPhotoGrid">
                        @forelse($car->images as $img)
                            <div class="photo-thumb" data-image-id="{{ $img->id }}" data-cover-url="{{ route('cars.images.setCover', ['image' => $img->id]) }}">
                                <img src="{{ asset('storage/'.$img->image_path) }}" alt="">
                                @if($img->is_cover ?? false)
                                    <span class="cover-tag">Cover</span>
                                @else
                                    <button type="button" class="set-cover-btn" data-url="{{ route('cars.images.setCover', ['image' => $img->id]) }}">Set cover</button>
                                @endif
                                <button type="button" class="remove-thumb" data-url="{{ route('cars.images.destroy', ['image' => $img->id]) }}" title="Delete photo"><i class="bi bi-x"></i></button>
                            </div>
                        @empty
                            <p class="empty-photos-note">No photos yet — add at least 5 below.</p>
                        @endforelse
                    </div>

                    <div class="photo-subhead">Add More Photos <span class="opt" style="text-transform:none;">(optional)</span></div>
                    ...
                    <div class="photo-count-row">
                        <span id="photoCountText" class="count ok">{{ $car->images->count() }} of 5 minimum photos total</span>
                    </div>
        ...
        <div class="submit-bar">
            <a href="{{ route('cars.show', ['car' => $car->id]) }}" class="cancel-link">Cancel</a>
...
<script>
    ...
    // ---------- existing photos: delete / set cover via AJAX ----------
    const existingPhotoGrid = document.getElementById('existingPhotoGrid');
    let existingCount = existingPhotoGrid.querySelectorAll('.photo-thumb').length;

    existingPhotoGrid.addEventListener('click', async (e) => {
        const removeBtn = e.target.closest('.remove-thumb');
        ...
        if (removeBtn) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    showToast('Could not delete this photo.', true);
                }
            } catch (err) {
        ...
        if (coverBtn) {
            try {
                const res = await fetch(coverBtn.dataset.url, {
                    method: 'POST',
                    headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' },
                });
                if (res.ok) {
                    existingPhotoGrid.querySelectorAll('.photo-thumb').forEach(thumb => {
                        const tag = thumb.querySelector('.cover-tag');
                        if (tag) {
                            tag.outerHTML = `<button type="button" class="set-cover-btn" data-url="${thumb.dataset.coverUrl}">Set cover</button>`;
                        }
                    });
                    coverBtn.outerHTML = '<span class="cover-tag">Cover</span>';
                    showToast('Cover photo updated.');
                } else {
                    showToast('Could not set this as the cover photo.', true);
                }
            } catch (err) {
                showToast('Network error — please try again.', true);
            }
    ...
    // ---------- new photo uploader (drag & drop, previews) ----------
    const MIN_PHOTOS = 5;
    ...
    function refreshPhotoCount() {
        ...
        photoCountText.textContent = total >= MIN_PHOTOS
            ? `${total} photos total`
            : `${total} of ${MIN_PHOTOS} minimum photos total`;
        photoCountText.className = 'count ' + (total >= MIN_PHOTOS ? 'ok' : 'low');
        checkPhotos.classList.toggle('done', total >= MIN_PHOTOS);
    }
    refreshPhotoCount();
    ...
    // ---------- guard: require at least 5 photos total before submit (matches update()'s server-side check) ----------
    document.getElementById('carForm').addEventListener('submit', function (e) {
        const total = existingCount + selectedFiles.length;
        if (total < MIN_PHOTOS) {
            e.preventDefault();
            dropzone.scrollIntoView({ behavior: 'smooth', block: 'center' });
            photoCountText.textContent = `Please have at least ${MIN_PHOTOS} photos total (${total} right now)`;
